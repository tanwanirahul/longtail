...
(function(win) {
	...
	function applyPatches(selectorText, patches) {
		...
		try {
			if(selectorMethod == Y.all)
			{
				elms = [];
				Y.all(domSelectorText).each(function(e){elms.push(Y.Node.getDOMNode(e))});
			}else
			{
	...
	selectorMethod = Y.all;
	if (selectorMethod && _css3_styles) {
		var cssText = patchStyleSheet(_css3_styles);
		var sts = doc.styleSheets[0]; // doc.getElementById('ieCSSStyles');
		sts.cssText = cssText;
	}
