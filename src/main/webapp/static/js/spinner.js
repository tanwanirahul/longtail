YUI.add("longtail-spinner", function(Y){
    ...
    Spinner.HTML_PARSER = {
        value: function (srcNode) {
            var val = parseFloat(srcNode.get("value"));
            return Y.Lang.isNumber(val) ? val : 0;
        }, 
        
        name: function (srcNode) {
            return srcNode.getAttribute("name");
        },
        
        max: function (srcNode) {
            var val = parseFloat(srcNode.getAttribute("max")); 
            return Y.Lang.isNumber(val) ? val : 1000;
        },
        
        min: function (srcNode) {
            var val = parseFloat(srcNode.getAttribute("min")); 
            return Y.Lang.isNumber(val) ? val : -1000;
        },
        
        minorStep: function (srcNode) {
            var val = parseFloat(srcNode.getAttribute("minorStep")); 
            return Y.Lang.isNumber(val) ? val : 1;
        },
        
        majorStep: function (srcNode) {
            var val = parseFloat(srcNode.getAttribute("majorStep")); 
            return Y.Lang.isNumber(val) ? val : 10;
        },
        
        strings: function (srcNode) {
            var strings = this.get("strings");
            strings.tooltip = srcNode.getAttribute("title");
        }
    };
    ...
    Y.extend(Spinner, Widget, {
        ...
        _onInputChange : function(e) {
        	...
        	if(e && e.type == "spinner")
        	{
        		// Self event. No Need to handle
        		return;
        	}
        	
        	var value = parseFloat(this.inputNode.get("value"));
        	
        	if(isNaN(value)) value = 0;
        	
            if (!this._validateValue(value)) {
                throw "Bad value for " + this.inputNode.get("name") + "=" + this.inputNode.get("value");
            }else
            {
        ...
        _renderInput : function() {
            ...
            if (!input) {
                ...
                input.setAttribute("name", this.get("name"));
            }
        ...
        _renderButtons : function() {
            ...
            var containerButton = Y.Node.create("<div></div>");
            containerButton.addClass(this.getClassName("container"));
            containerButton.appendChild(inc);
            containerButton.appendChild(dec);
            contentBox.appendChild(containerButton);
            
            this.incrementNode = inc;
            this.decrementNode = dec;
        },

        /**
         * Utility method, to create a spinner
         */
        ...
        _uiSetValue : function(val) {
            this.inputNode.set("value", val.toFixed(1));
            
            if(this.get("watching"))
            {
            	this.inputNode.simulate("change",{type: "spinner"});
            }
        },
...
},"3.3.0",{requires: ["event-key", "widget"]});